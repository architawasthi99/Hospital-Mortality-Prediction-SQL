...
SELECT
    CASE
        ...
        ELSE '80 and above'
    END AS age_group,
    ROUND(AVG(apache_4a_hospital_death_prob),3) AS average_death_prob           --ARCHIT AWASTHI
FROM patient_survival.ps_data
GROUP BY age_group;
...
SELECT
    ROUND(AVG(CASE WHEN gender = 'M' THEN weight END),2) AS avg_weight_male,                         --ARCHIT AWASTHI
...
SELECT
    ...
    ROUND(AVG(bmi),2) AS average_bmi
FROM patient_survival.ps_data
WHERE bmi IS NOT NULL                        --ARCHIT AWASTHI
...
SELECT
    ...
    CASE
        ...
        WHEN bmi >= 18.5 AND bmi < 25 THEN 'Normal'                        --ARCHIT AWASTHI
